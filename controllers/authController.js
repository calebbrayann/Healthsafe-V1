...
export async function registerMedecin(req, res) {
  try {
    const { nom, prenom, email, password, telephone, specialite, numero_licence, hopital } = req.body;

    if (!nom || !prenom || !email || !password || !telephone || !specialite || !numero_licence || !hopital) {
      return res.status(400).json({ message: 'Tous les champs sont requis.' });
    }
    ...
    // Envoi email admin
    try {
      await envoyerEmail('notificationAdmin', process.env.EMAIL_ADMIN, { nom, email });
    } catch (err) {
      console.error('Erreur envoi email admin:', err.message);
    }
    ...
    try {
      await envoyerEmail('bienvenueMedecin', email, {
        ...
        hopital,
        message: 'Votre inscription a été enregistrée. En attente de validation par l’admin.'
      });
    } catch (err) {
      console.error('Erreur envoi email médecin:', err.message);
    }

    return res.status(201).json({ message: 'Inscription médecin réussie. En attente de validation admin.' });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: 'Erreur serveur.' });
  }
...
export async function requestResetPassword(req, res) {
  try {
    ...
    const utilisateur = await prisma.utilisateur.findUnique({ where: { email } });
    if (!utilisateur) return res.status(404).json({ message: 'Email non trouvé.' });

    const resetToken = jwt.sign({ userId: utilisateur.id }, process.env.JWT_SECRET, { expiresIn: '30m' });

    await prisma.resetToken.create({
      data: { utilisateurId: utilisateur.id, token: resetToken, expiration: new Date(Date.now() + 30 * 60 * 1000) }
    });
...
export async function resetPassword(req, res) {
  try {
    ...
    const resetTokenEntry = await prisma.resetToken.findUnique({ where: { token } });
